...
function AgentVerifyEmailContent() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:4001'}/api/agent/verify-email`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        setSuccess(true);
        setTimeout(() => {
          router.push('/agent/login?verified=true');
        }, 2000);
      } else {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:4001'}/api/agent/resend-otp`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">
              Your agent account has been verified. You can now sign in.
            </p>
...
export default function AgentVerifyEmailPage() {
  return (
    <Suspense fallback={
