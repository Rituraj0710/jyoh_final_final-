...
export default function Staff4WorkReportPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/staff/4/reports?${queryParams}`, {
  ...
  const handleSubmitReport = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/staff/4/reports`, {
        method: 'POST',
        headers: headers,
